...
def create_account():
    ...
    try:
        ...
        query = """
        INSERT INTO dbo.usuario (nombre, apellido, edad, carnet, contraseña)
        VALUES (?, ?, ?, ?, ?)
        """
        cursor.execute(query, (name, last_name, age, id_card, password))
        conexion.commit()
...
def search_buses():
    global point_origin_input, point_destination_input, departure_date_button, passengers_entry, passenger_class_input
    ...
    if not passengers.isdigit() or not (1 <= int(passengers) <= 60):
        messagebox.showerror("Error", "El Numero de Pasajeros debe ser un número entre 1 y 60")
        return
    ...
    try:
        cursor = connection.cursor()
        query = """
        SELECT b.bus_id, b.nombre, c.chofer_id, c.nombre AS chofer_nombre, 
               b.fecha_salida, b.fecha_retorno, COUNT(r.bus_id) AS asientos_ocupados
        FROM dbo.bus AS b
        JOIN dbo.chofer AS c ON b.chofer_id = c.chofer_id
        LEFT JOIN dbo.reserva AS r ON b.bus_id = r.bus_id
        WHERE b.ruta_id IN (
            SELECT ruta_id FROM dbo.ruta 
            WHERE dep_inicio = ? AND dep_final = ?
        )
        AND b.fecha_salida = ?
        GROUP BY b.bus_id, b.nombre, c.chofer_id, c.nombre, b.fecha_salida, b.fecha_retorno
        HAVING COUNT(r.bus_id) <= (60 - ?)
        """
        cursor.execute(query, (origin, destination, departure_date, return_date, int(passengers), passenger_class))
        buses = cursor.fetchall()
        if not buses:
            messagebox.showinfo("Sin Resultados", "No se encontraron Buses Disponibles")
            return
        buses = []
        results_frame = make_show_results(buses, passengers, origin, destination)
...
def make_start_frame():
    ...
    try:
        ...
        print(f"Error al cargar el isotipo: {e}")
    # Texto
...
def make_terms_frame():
    ...
    for title, content in sections:
        section_frame = tk.Frame(content_frame, bg="#F1F2F6")
...
def make_register_frame():
    ...
    title_label.pack(pady=50)
    # Botón de Regreso
    show_back_button(register_frame)
...
def make_show_results(buses, passengers, origin, destination):
    ...
    if not buses:
        no_results_label = tk.Label(
            results_frame,
            text="No se encontraron Buses Disponibles",
            bg="#F1F2F6",
            font=("Canva Sans", 12)
        )
        ...
        for idx, bus in enumerate(buses):
            ...
            block_frame = tk.Frame(results_frame, bg="#F1F2F6", padx=10, pady=10)
...
Asientos Disponibles: {60 - asientos_ocupados}
            ...
            if idx < len(buses) - 1:
                separator = tk.Frame(results_frame, bg="#7732FF", height=2, width=300)
...
"""Funcion para llevar al usuario al Frame Inicial"""
