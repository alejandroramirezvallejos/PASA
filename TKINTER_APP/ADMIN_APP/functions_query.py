...
# adicionan cosas
#addiciona bus
def add_bus(chofer_id,ruta_id,nombre,fecha_sal,fecha_ret):
    conexion=make_connection()
    cursor=conexion.cursor()
    cursor.execute(f"""INSERT INTO bus (bus_id, chofer_id, ruta_id,nombre, fecha_salida,fecha_retorno)
VALUES ({obtain_pk(cursor,"bus")},{chofer_id},{ruta_id},'{nombre}','{fecha_ret}','{fecha_sal}');
""")
    conexion.commit()
#adicciona chofer
